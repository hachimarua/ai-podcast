...
4. エンディング（採用形式に従い要点を短くまとめ、最後は「今回は以上です。それではまた次回お会いしましょう。安全運転でいってらっしゃい。」などの5〜10秒程度の短い挨拶で番組を締めくくる。長いまとめや定型的な励ましは不要）
...
- 「必須」は音声では「ひっす」、「案」は「あん」のように、音声合成で誤読されやすい語は必要に応じて明瞭に読める表記にし、「Hugging Face」は「ハギングフェイス」、「DeepMind」は「ディープマインド」、「TechCrunch」は「テッククランチ」、「GitHub Actions」は「ギットハブアクションズ」、「Cloudflare」は「クラウドフレア」のように表記してください。発音が崩れやすい英語固有名詞を途中で省略しないでください。
...
def ensure_script_greetings(
    ...
    if not closing_present:
        ...
        closing_text = (
            f"{closing_speaker}：今回は以上です。それではまた次回お会いしましょう。安全運転でいってらっしゃい。"
        )
...
- 会話の最後は要点を短くまとめた上で、短い終了挨拶（例:『今回は以上です。それではまた次回お会いしましょう。安全運転でいってらっしゃい。』）で締める。
""".strip()
...
- 会話の最後は、短い終了挨拶（例:『今回は以上です。それではまた次回お会いしましょう。安全運転でいってらっしゃい。』）で自然に締めくくる。
""".strip()
    raise EpisodeFormatError("episode format must be daily or lab")
...
def build_prompt_content(
    ...
    content += (
        ...
        "日本語対話台本を作成してください。"
        "番組冒頭には短い開始挨拶（番組名と挨拶、5〜10秒程度）、最後には短い終了挨拶（例:『今回は以上です。それではまた次回お会いしましょう。安全運転でいってらっしゃい。』）を必ず含めてください。\n"
    )
    if selected_terms:
        content += "過去にユーザーが学んだ内容（学習メモ本文に記載されている内容）をおさらいしながら、最新情報と結びつけて解説してください。\n"
    else:
        content += "復習メモの代わりに、提供された最新ニュースから聞く価値の高いものを選び、採用した各ニュースを十分に説明してください。1件で十分なら1件だけで構いません。\n"
    if episode_format == "daily":
