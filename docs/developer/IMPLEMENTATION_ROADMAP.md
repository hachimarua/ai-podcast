...
- Antigravity Sidecarが毎日06:30、または06:30を過ぎて未実行のまま起動・復帰した直後に、独立した子プロセスとして取り込む。品質通知処理自身は`.env`を読み込まない。
...
**予定開発（Phase 0〜6）完了 — Phase 6の入力経路を実運用へ接続した段階**
...
- Sidecarは毎日06:30に確認し、予定時刻にMacがスリープ中なら復帰後30秒以内に当日分を1回だけcatch-upする。トライアル期間中は正常・要確認・監査未完了・生成結果未確認のいずれも日次監査報告として`agentapi new-conversation`へ送る。
...
- 2026-07-23追記: 長時間sleepによる時刻ずれを防ぐため、30秒ごとの壁時計確認と永続的な同日重複防止へ変更した。06:30にスリープ中なら復帰後30秒以内にcatch-upし、翌日の予定時刻はずらさない。
...
### 学習ノート取り込みの実運用接続（2026-07-28）

詳細は[学習ノート取り込み](./LEARNING_NOTE_INTAKE.md)を正本とする。

- Phase 6のアダプターは2026-07-06から常駐していたが、`20_Dev_開発/Learning/`が空のまま3週間動作せず、Gem→Notion受信箱の転記が手作業で残っていた。
- `No content found`・`情報なし`の行は、本文が空の受信箱ページに対し旧実装がタイトルだけをGeminiへ渡していたことが原因だった。実データで子ブロック0件を確認した。
- `fetch_page_content`が`table`を読まず、表として貼り付けた内容が落ちていたことも確認し、`table_row`と入れ子ブロックの読み取りを追加した。
- 取り込み前後の形式をそろえ、Gemが出す「保存用ブロック」をそのまま最終形式とする方針へ変更した。
- `scripts/save_learning_note.py`を追加し、クリップボードだけを読んで`ai_radio: true`付きノートを保存、続けてNotion受信箱へ取り込む。先頭が`# 用語名`でない、空、2万文字超は保存しない。
- `obsidian_inbox_adapter.py`の分割を行境界に変更し、Notion往復でMarkdownが壊れないようにした。
- `process_inbox.py`は`Obsidian｜`で始まる受信箱アイテムをAIに通さず決定論的に登録し、空本文とプレースホルダーの用語・要約は登録せず受信箱へ残す。放送は止めない。
- 実データで通し確認した。`discovered=1 pending=1 imported=1`、再実行は`pending=0`、元ノートのハッシュ不変、Notion往復後に見出し5・箇条書き12・段落1で復元。
- ログには受信箱内の位置だけを出し、タイトル・本文・ページIDを出さない既存方針を維持した。

### 受け口を広げ、選別を処理側へ移す（2026-08-01）

- 上記の「先頭が`# 用語名`でなければ保存しない」を撤回した。Gem以外のチャット（Codexの
  サイドチャット等）の回答は素のテキストで、良い内容ほど門前払いされる逆転が起きたため。
- 保存段階で断るのは空と2万文字超だけにした。見出しの無い貼り付けは1行目から仮の題を作り、
  frontmatterへ`format: raw`を立てて保存する。
- `obsidian_inbox_adapter.py`は`format: raw`のノートを`Obsidian未整形｜`で受信箱へ入れる。
  `process_inbox.py`はこれを手入力メモと同じくGeminiで用語名・要約を付けてから登録する。
- 学習素材かどうかの判定は`is_learning_material`としてGeminiの構造化出力へ追加した。門戸は
  広く取り、日記・予定・買い物メモなど明らかに無関係なものだけ落とす。落としたものは削除
  せず受信箱に残すため、誤判定でも本文は失われない。
- 自動テスト144件成功。
...
- 2026-07-28: Phase 6の入力経路を実運用へ接続。`scripts/save_learning_note.py`、Notion往復の行境界分割、Obsidian昇格ノートの決定論的登録、プレースホルダーの登録拒否を追加。139テスト成功。
